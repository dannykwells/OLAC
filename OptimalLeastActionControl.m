%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%This is my implementation of optimal least action control. 
...
%Calculate the action values for each pair of fixed points.
for i=1:NumFP 
    for j=1:NumFP
        if i==j
            ActionVal = Inf; 
...
%For saving values of the optimization as it proceeds. 
global history
stop = false; 
...
%homotopy method of a stable fixed point. 
...
%Since we're just following a stable fixed point, it's easy to just run the
%forward ODE. 
%Other approaches to do this could use a Newton method (fsolve) to continue
%the point as well. Of course, pseudo-arclegth continuation woudl be even
%more fancy, but probably isn't required here. 
for i=1:N
...
%This is the gradient calculation for the objective functional.
